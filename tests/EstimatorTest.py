...
def test_cli_subcommand_show():
    from sklearn_porter.cli.command.show import main

    out = main(dict(), silent=True)
    assert 'DecisionTreeClassifier' in out
    assert 'Java' in out
    assert 'JavaScript' in out

    out = main(dict(language='c'), silent=True)
    assert 'DecisionTreeClassifier' in out
    assert 'Java' not in out
    assert 'JavaScript' not in out

    with pytest.raises(exception.InvalidLanguageError):
        main(dict(language='i_n_v_a_l_i_d'), silent=True)


def test_cli_subcommand_port():
    from sklearn_porter.cli.command.port import main

    out = main(
        dict(model=str(SERIALIZED_MODEL), skip_warnings=True), silent=True
    )
    assert 'This file is generated by' in out

    out = main(
        dict(
            model=str(SERIALIZED_MODEL),
            langauge='java',
            template='combined',
            skip_warnings=True
        ),
    ...
    out = main(
        dict(language='js', model=str(SERIALIZED_MODEL), skip_warnings=True),
        silent=True
    )
    assert 'node' in out and 'require.main === module' in out

    if SKLEARN_VERSION[:2] != (0, 19):
        with pytest.raises(UserWarning):
            main(dict(model=str(SERIALIZED_MODEL)), silent=True)


def test_cli_subcommand_save(tmp_root_dir: Path):
    from sklearn_porter.cli.command.save import main
